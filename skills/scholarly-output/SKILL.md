...
- Quoting a formula from a parsed PDF: preserve its mathematical meaning exactly, while still applying the required transport-safe spelling. In particular, convert source `\{` / `\}` or any `\left` / `\right` brace form to direct `\lbrace` / `\rbrace`; this syntax-only conversion is required, not a change to the quoted mathematics. If a transcription looks suspicious (dropped `<<` / `>>`, confusions like `BF16` vs `BFl6`), verify against the corresponding crop first — see the pdf-reader skill.
